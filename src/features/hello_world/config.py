...
class HelloWorldConfig(BaseSettings):
    ...
    # Server settings
    server_name: str = Field(default="Hello World MCP Server", description="Name of the MCP server")
    ...
    # Transport settings
    transport_type: TransportType = Field(default=TransportType.STDIO, description="Transport mechanism to use")
    host: str = Field(default="localhost", description="Host to bind to for SSE/WebSocket")
    ...
    enable_server_info_tool: bool = Field(default=True, description="Enable server info tool")
    enable_status_resource: bool = Field(default=True, description="Enable status resource")
    enable_usage_stats_resource: bool = Field(default=True, description="Enable usage stats resource")
    enable_formal_greeting_prompt: bool = Field(default=True, description="Enable formal greeting prompt")
    enable_casual_greeting_prompt: bool = Field(default=True, description="Enable casual greeting prompt")
    ...
    reload_on_change: bool = Field(default=False, description="Reload server on file changes (development)")
